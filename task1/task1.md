...
Для использования уязвимости необходимо загрузить файл и изменять его используя свой resizeParam, вывод программы можно получить через \<filename\>.mut.png
...
Удалить проверку шаблона и попробовать провести argument injection не вышло - ProcessBuilder передаёт аргументы напрямую в argv, минуя обработку оболочкой, поэтому convert ожидал после -resize значение данного параметра и завершал свою работу с ошибкой, если передавались аргуметы начинающиеся на "-" или аналогичные "100x100 -flip".
